  ...
  return (
    <div className={`pointer-events-none relative z-10 h-0 ${className}`} aria-hidden="true">
      <motion.div
        ...
        className="absolute left-1/2 top-0 h-px w-2/3 bg-gradient-to-r from-transparent via-cyan-300 to-transparent"
      />
      <motion.div
        animate={{ opacity: [0.3, 0.7, 0.3] }}
        transition={{ duration: 4, repeat: Infinity, ease: 'easeInOut' }}
        className="absolute left-1/2 top-0 h-32 w-[min(36rem,100vw)] -translate-x-1/2 -translate-y-1/2 rounded-full bg-teal-400/30 blur-3xl"
      />
    </div>
  )
}
